<%@ Page Language="VB" Trace="False" Debug="False" MasterPageFile="lang.master" StylesheetTheme="Lang"
    AutoEventWireup="false" CodeFile="SRDims.aspx.vb" Inherits="Lang_SRDims" ValidateRequest="False"
    Title="Stress/Recovery Dimensions" %>

<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    <asp:SqlDataSource ID="rsA" runat="server" ConnectionString="<%$ ConnectionStrings:c7tudes %>"
        SelectCommand="SELECT AutoID,  [LanguageID], [SRDimNo], SRDimName, SRDimDesc, [EnglishSRDimName], EnglishSRDimDesc FROM [Lang_SRDimensions] WHERE ([LanguageID] = @LanguageID) and  ((@txtSearch='zz') OR (CharIndex(@txtSearch, IsNull(SRDimName, '')+ ' -- ' +IsNull(SRDimDesc, '')+ ' -- ' +IsNull(EnglishSRDimName, '')+ ' -- ' +IsNull(EnglishSRDimDesc, ''))>0)) ORDER BY [SRDimNo]">
        <SelectParameters>
    ...
    <div class="Section">
        Stress/Recovery Dimensions&nbsp; &nbsp; &nbsp; (Current Search:
    ...
    <asp:GridView ID="tabA" runat="server" Width="350px" CellPadding="4" DataSourceID="rsA"
        ...
        <Columns>
            <asp:TemplateField HeaderText="Row ID">
                <ItemTemplate>
                    <asp:Label ID="SRDimNo" runat="server" Text='<%# Eval("SRDimNo") %>'></asp:Label>
            ...
            <asp:TemplateField HeaderText="Dimension Name">
                <ItemTemplate>
                    <asp:Label ID="EnglishSRDimName" runat="server" Text='<%# Eval("EnglishSRDimName") %>'></asp:Label>
                    <br />
                    <asp:TextBox ID="SRDimName" runat="server" Text='<%# Bind("SRDimName") %>' SkinID="1"
                        Columns="80"></asp:TextBox><br />
                    Max. Allowed:
                    <asp:Label runat="server" ID="maxlength1" ForeColor="Green" Text='<%# Len(Eval("EnglishSRDimName")) %>'></asp:Label>&nbsp;
                    &nbsp; Chars Left:
